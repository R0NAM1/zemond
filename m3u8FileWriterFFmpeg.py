...
global killSignal, frameQueueEmptyTracker, discardFrames
...
def writeFramesToQueue(inputContainer, rtspFrameQueue):
    global killSignal, discardFrames
    try:        
...
def writeFromQueueToContainerForSeconds(rtspFrameQueue, outputAudio, outputVideo, outputContainer, videoTimeBase, secondsToWrite, segmentWriting, m3u8Playlist, m3u8AbsolutePath):
    ...
    # Generate time left in minute
    currentTime = datetime.datetime.now()
    # print(currentTime.second)
    timeDiff = 60 - currentTime.second
    
    # print("Segment length will be " + str(timeDiff))
...
def mainLoopingLogic():
    ...
    # Wait for minute to rollover
    currentTime = datetime.datetime.now()
    timeLeftInMinute = 60 - currentTime.second
    
    # Wait until minute rolls over minus 1 second
    # print("Waiting " + str(timeLeftInMinute))
    time.sleep(timeLeftInMinute)
    discardFrames = False
    ...
    maxSegmentsCalculated = (retentionPeriodInSeconds // 60)
    ...
    while True:        
        ...
        if segmentAmount < maxSegmentsCalculated:
            ...
            segment = m3u8.Segment(uri=videoFileAbsolutePath, duration=60)
        ...
        # If the playlist has met the segment quota, find the oldest segment by EXT-X-PROGRAM-DATE-TIME and grab URI to write to, and make newest segment
            ...
            for currentSegmentItteration in m3u8PlaylistObject.segments:
                ...
                if currentSegmentItteration.program_date_time.replace(tzinfo=None) < (m3u8PlaylistObject.segments[oldestSegmentIndex]).program_date_time.replace(tzinfo=None):
            ...
            segment = m3u8.Segment(uri=storedUriFromSegment, duration=60)
