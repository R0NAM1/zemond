import asyncio, ast, json, time, cryptocode
...
async def singleWebRtcStart(thisUUID, dockerIP, cameraName, request):
    try:
        ...
        # Media source being local docker container and is just RTSP rebroadcasted locally
        # TAKES TIME, See if can be sped up
        ...
        # After event handlers mentioned and video source prepared, add video and audio to tracks.
...
async def monWebRtcStart(request, thisUUID, dockerIpArray, formatCamArray):
    try:
        ...
        # Very slow and CPU dependant right now, replace MediaPlayer with pyav and add gpu support
        # -hwaccel auto should fix it, need to test later, but WebRTC Is functional, so focus on client for now and finish general monitor
        # stuff before fixing, need to before map view though, will exhibit same issue.
                
        for i, cam in enumerate(formatCamArray):
            print(str(i) + ": " + cam + " : " + dockerIpArray[i])    
            player = MediaPlayer("rtsp://" + dockerIpArray[i] + ":8554/cam1")

            if (player.video):
                webRtcPeer.addTransceiver(player.video, direction='sendonly')
            # Don't need to load audio, goes with video
            
            # for sender in webRtcPeer.getSenders():
            #     if sender.track == player.video:
            #         print("Video transceiver added successfully")
